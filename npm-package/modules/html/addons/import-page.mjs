...
export async function beforeAsync (iterator, htmlDocument, page, config, api) {
  ...
  global.context[id] = global.context[id] || { page, config, api }
}
...
module.registerHooks({
  ...
  load (url, context, nextLoad) {
    ...
    const ids = url.slice(url.lastIndexOf('#') + 1).split('|').reverse()
    ...
    const id = ids.find(id => `${id}` in global.context) ||
      ids.find(id => `#${id}` in global.context) ||
      Object.keys(global.context)[Object.keys(global.context).length - 1]
    ...
    for (const key of submodules) {
      if (url.startsWith(`page:${key}`)) {
        return {
          format: 'module',
          shortCircuit: true,
          source: `export default global.context['${id}']['${key}']`
        }
      }
    }
