...
function enableCommands(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand('pages.serve', async () => {
    ...
    if (!workspace) {
      terminal.sendText('npx @plushveil/pages serve ', false)
    } else {
      terminal.sendText(`npx @plushveil/pages serve "${workspace}"`, true)
    }
  })
  context.subscriptions.push(disposable)
}

function enableConfigurationWatcher(context: vscode.ExtensionContext) {
...
function enableLanguageServer(context: vscode.ExtensionContext) {
  client = new languageClient.LanguageClient(
    ...
    {
      debug: { args: ['--inspect=6009', serverPath, '--stdio'], command: process.execPath },
      run: { args: [serverPath, '--stdio'], command: process.execPath },
    },
    {
      documentSelector: [{ language: 'pages', scheme: 'file' }],
    },
  )
...
function enableEmmet() {
  ...
  const currentIncludeLanguages = (emmetConfig.get('includeLanguages') || {}) as Record<string, string>
...
async function connectToLanguageServer(context: vscode.ExtensionContext) {
  ...
  const triggerSuggestListener = vscode.workspace.onDidChangeTextDocument((event) => {
    ...
    if (editor.document.languageId !== 'pages') {
      return
    }
    ...
    if (isInTemplateLiteral || isInScript) {
      ...
      if (triggerCharacters.includes(insertCharacter)) {
        triggerSuggest()
      }
      return
    }
